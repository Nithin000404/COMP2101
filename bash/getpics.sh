...
# making  a picture directory
test -d ~/Pictutres || mkdir ~/Pictures

#downloading a tarfile of picturesto our Pictures directory
test -f ~/Pictures/pics.tgz || wget -q -o ~/Picturess/pics.tgz http:??zonzorp.net/pics.tgz

#unpacing the downloaded tar file if it is there ,then delete the local copy of the tarfile
test -f ~/Pictures/pics.tgz && unzip -d ~/Pictures -o -q ~/Pictures/pics.tgz && rm ~/Pictures/pics.tgz

#make a repot on what we have in the pioctures directory
test -d ~/Pictures && cat <<EOF
Found $(find ~/Pictures -type f|wc -l) files in the Pictures directory.
The Pictures directory uses $(du -sh ~/Pictures|awk '{print $1}') space on the disk.
EOF
